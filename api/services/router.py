...
from api.services.requests import ALLOWED_EXTENSIONS, MAX_FILES, UploadedServicesFiles
from api.common.responses import AckResponse
from core.extractor import extract_text_from_pdf
from services.services.gemini_client import extract_services_fields
from services.services.verifier import build_verification_prompt
from services.services.xlsx_builder import build_xlsx
from core.mailer import send_error_email, send_invoice_email
from core.verifier import verify_extraction
from utils.services.gsheet import load_field_config
...
def _process_and_email(
    ...
    for filename, file_bytes in file_items:
        try:
            ...
            result = extract_services_fields(text, config_df, model)
    ...
    try:
        ...
        send_invoice_email(
            ...
            subject     = f"Services Invoices Ready — {len(successful)} invoice(s) processed",
            filename    = "services_invoices.xlsx",
        )
    except Exception as e:
...
async def process_services(
    ...
    return AckResponse(
        queued=True,
        message=f"{n} service invoice(s) queued for processing{skipped_note}. Results will be emailed to {', '.join(recipients)}.",
    )
